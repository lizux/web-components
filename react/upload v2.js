...
export default class FileUpload extends React.Component {
    ...
    onDrop(evt) {
        ...
        if (evt.dataTransfer) {
            action = 'drop';
            files = evt.dataTransfer.items || evt.dataTransfer.files;
        } else if (evt.target) {
        ...
        fileArr.forEach((item, i) => {
            if (action === 'drop') {
                this.parse(item.webkitGetAsEntry());
            } else {
                this.handle(item);
            }
        });
    ...
    upload(url, param, callback, fallback) {
        ...
        fileBlob.forEach((value) => {
            data.append(this.props.fileName, value);
        });
        let config = {
        ...
        request.post(url, data, config).then((response) => {
            ...
            if (result.meta && result.meta.code === 200) {
                ...
                console.log(result.meta.errmsg || result.meta.error_message);
    ...
    componentDidMount() {
        let input = ReactDOM.findDOMNode(this._fileInput);
                        ...
                        if (!this.props.oneClick && thumbnails.length > 0) {
                            return thumbnails.map((item, i) => (
                                <div key={i} className="hoverRemove" style={styles.box} title={item.name} onClick={this.remove.bind(this, i)}>
                                    ...
                                    <a href style={styles.close} title="remove">&times;</a>
